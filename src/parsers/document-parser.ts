...
function extractSection(markdown: string, heading: string): string | null {
  ...
  const re = new RegExp('##\\s+\\d*\\.?\\s*' + esc + '[\\s\\S]*?(?=\\n##\\s|$)', 'i')
...
function extractSubSection(block: string, heading: string): string | null {
  ...
  const nextKw = after.search(/\n(?:职责|建议实现|字段|要求|推荐数据|```)/i)
  const section = nextKw === -1 ? after : after.slice(0, nextKw)
...
function extractModules(markdown: string): ModuleDefinition[] {
  ...
  const blocks = body.split(/###\s+模块\s*\d+[：:]\s*/)
...
function parseModuleBlock(block: string): ModuleDefinition | null {
  ...
  if (fieldBlock) {
    for (const line of fieldBlock[1].split('\n')) {
      ...
      if (fm) {
        fields.push({ name: fm[1], type: fm[3].trim(), required: fm[2] !== '?' })
      }
    }
  }
